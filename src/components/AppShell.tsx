...
import { Map, Flag, User, ScanLine, Volume2, VolumeX, LogOut } from "lucide-react";
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const items: Array<{ to: "/scan" | "/map" | "/quests" | "/profile"; icon: typeof Map; label: string; hero?: boolean }> = [
          ...
          <div className="mx-auto max-w-6xl px-4 pb-3">
            <div className="rounded-2xl border-2 border-gold bg-gradient-to-r from-gold/25 to-primary/15 px-3 py-2 shadow-sm">
              ...
              <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-card">
                <div className="h-full bg-gradient-to-r from-primary to-gold transition-all" style={{ width: `${Math.min(100, (uq.correct_scans / uq.target_count) * 100)}%` }} />
              </div>
            </div>
          </div>
        )}
              ...
              return (
                <li key={it.to} className="-mt-6 flex-shrink-0">
                  <Link
                    ...
                    aria-label={it.label}
                  >
                    <span className={`grid size-16 place-items-center rounded-full border-4 border-card bg-gradient-to-br from-primary to-gold text-primary-foreground shadow-[0_10px_24px_-6px_oklch(0.7_0.15_25/0.6)] ${active ? "scale-105" : ""} transition-transform`}>
                      <Icon className="size-7" />
                    </span>
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${active ? "text-primary" : "text-muted-foreground"}`}>{it.label}</span>
                  </Link>
                </li>
              );
            }
