  ...
  return (
    <div className="mx-auto max-w-3xl">
      <header className="mb-6">
        <p className="text-xs uppercase tracking-[0.25em] text-muted-foreground">{t("journal_title")}</p>
      ...
      <section className="paper-card p-5">
        <div className="ornament-rule mb-4 text-xs uppercase tracking-[0.25em] text-muted-foreground">
          <span>{lang === "bm" ? "Artifak Ditemui" : "Artifacts Discovered"} · {data.scans.length}/{TOTAL_ARTIFACTS}</span>
        </div>
        {data.scans.length === 0 ? (
            ...
            {data.scans.map((s, i) => {
              ...
              return (
                <li key={s.artifact_id} className="flex gap-3">
                  <span className="mt-0.5 grid size-7 shrink-0 place-items-center rounded-full border border-border text-xs font-display">{i + 1}</span>
                  <div className="flex-1">
                    <p className="font-display text-base">{lang === "bm" ? a.name_bm : a.name_en}</p>
                    <p className="text-xs text-muted-foreground">{lang === "bm" ? a.era_bm : a.era_en} · {lang === "bm" ? a.origin_bm : a.origin_en}</p>
                    <p className="mt-1 line-clamp-2 text-sm text-foreground/80">{lang === "bm" ? a.description_bm : a.description_en}</p>
                  </div>
      ...
      {data.quests.length > 0 && (
        <section className="paper-card mt-6 p-5">
          ...
          <ul className="space-y-1 text-sm">
            {data.quests.map((q) => (
              <li key={q.id}>· {lang === "bm" ? q.name_bm : q.name_en} <span className="text-primary">+{q.exp_reward}</span></li>
            ))}
          </ul>
        </section>
      )}
