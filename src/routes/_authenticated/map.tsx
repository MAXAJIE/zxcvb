...
function MapPage() {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
            ...
            {artifacts.map((a) => {
              ...
              return (
                <g
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openArtifact(a); } }}
                  style={{
                ...
                >
                  {isScanned ? (
                    <>
                      {pulsing && (
                        <circle cx={p.x} cy={p.y} r={4.5} fill="none" stroke={meta.color} strokeOpacity={0.6} strokeWidth={0.6}>
                  ...
                  return (
                    <li key={a.id} className="relative">
                      <button
                        type="button"
                        onClick={() => openArtifact(a)}
                        aria-label={done ? (lang === "bm" ? a.name_bm : a.name_en) : t("map_locked_pin")}
                      ...
                      >
                        <div className={`aspect-square overflow-hidden rounded-2xl border-2 ${done ? "border-primary/40 group-hover:border-primary" : "border-dashed border-border"} bg-accent/40`}>
                          {done && a.image_url ? (
      ...
      {selected && (
        <ArtifactModal
          result={toReadOnlyResult(selected)}
